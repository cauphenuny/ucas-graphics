...
template <typename Derived> struct HalfEdgeMesh {
    template <typename... Args> Vertex createVertex(Args&&... args) {
    ...
    template <typename... Args> Edge createEdge(Args&&... args) {
    ...
    template <typename... Args> Face createFace(Args&&... args) {
    ...
    [[nodiscard]] auto edges() const { return ElementRange<EdgeElement>(m_edges); }

    [[nodiscard]] auto vertices() const { return ElementRange<VertexElement>(m_vertices); }

    [[nodiscard]] auto faces() const { return ElementRange<FaceElement>(m_faces); }

    [[nodiscard]] auto halfEdges() const { return ElementRange<HalfEdgeElement>(m_half_edges); }

    [[nodiscard]] int index(Vertex v) const { return v->index; }

    [[nodiscard]] int index(Edge e) const { return e->index; }

    [[nodiscard]] int index(Face f) const { return f->index; }
...
protected:
    ...
    template <typename Element> struct ElementRange {
        ...
        struct Iterator {
            ...
            ElementObserver operator*() const { return mystl::make_observer(it->get()); }
        ...
        [[nodiscard]] Iterator begin() const { return {.it = elements.begin()}; }

        [[nodiscard]] Iterator end() const { return {.it = elements.end()}; }
...
private:
    ...
    [[nodiscard]] const Derived& derived() const { return *static_cast<Derived*>(this); }
