...
public:
    ObjectPDF(const Samplable& objects, const Point3& origin) : objects(objects), origin(origin) {}
    double value(const Vec3& direction) const override {
...
public:
    MixturePDF(std::unique_ptr<Vec3PDF> p0, std::unique_ptr<Vec3PDF> p1, double w1 = 0.5)
